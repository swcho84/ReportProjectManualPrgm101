...
FileWordStatus ComparingSameData(FileWordStatus fpIn1, FileWordStatus fpIn2)
{
  ...
  while (nCounter < fpIn1.nSizeFp)
  {
    for (unsigned int i = 0; i < fpIn2.nSizeFp; i++)
    {
      ...
      {
        fpRes.strFp[i] = "same";
        fpRes.strSame[nSameCounter] = fpIn1.strFp[nCounter];
        nSameCounter++;
      }
    }
    nCounter++;
  }
  fpRes.nSameSize = nSameCounter;
  return fpRes;
}
...
int main()
{
  ...
  int nFileStatus = CheckFileStatus(fpIn1) + CheckFileStatus(fpIn2);
  if (nFileStatus != 2)
  {
  ...
  for (unsigned i = 0; i < fpOutStatus.nSameSize ; i++)
    printf("Same:[%d]%s\n", i, fpOutStatus.strSame[i]);
